...
| restart/cold-start safety | `pass_with_caveat` | `raw/restart-cold-start-probe.sanitized.json` | Read-only dry-run cold-start booted without persisted state/cursor and left public exposure zero; exit 124 was the expected 75s timeout signal, not a masked failure; not a process-level cursor-resume proof. |
