...
Verdict: `FULL_GREEN` **with caveats**

Reason: Live moneyline MM canary posted bounded quotes, completed one controlled fill, scored, settled, claimed winning positions, and ended with zero target exposure; caveats are limited to the not-run restart/cold-start probe, final settlement confirmation via indexer/protocol convergence after the CLI read omitted winSide/settledAt, and unrelated out-of-scope flow-wallet positions.
...
| `dry-run-quote-loop` | `proven_synthetic_only` | raw/mm-dryrun-summary.sanitized.json | Dry-run proved exactly target 41/30 and 0.10 USDC quote sizes; this proof is synthetic-only by nature and made no writes. |
...
| `restart-cold-start-safety` | `not_applicable` | — | Not run for this salvage; final zero-exposure and process checks were run instead. |
