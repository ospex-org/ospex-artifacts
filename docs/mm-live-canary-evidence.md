...
Of the five labels, only `FULL_GREEN` asserts a completed postgame lifecycle. The other three (`GREEN_LIVE_WINDOW_POSTGAME_DEFERRED` and both AMBER labels) describe a run that stopped in the live window, so none may carry a *successful* postgame transaction category (`score-request`, `score-callback`, `settle`, `claim`) — a disclosed *reverted* postgame attempt is still allowed.
...
- Successful-category presence must match the verdict: no successful postgame categories (`score-request`, `score-callback`, `settle`, `claim`) under any non-`FULL_GREEN` verdict; no successful fill (`match-commitment`/`seed-match`) under `AMBER_QUOTED_NO_FILL`; and a successful settle plus successful score evidence under `FULL_GREEN`. Reverted entries never count toward (or against) these checks, so disclosed failed attempts do not block an honest verdict.
