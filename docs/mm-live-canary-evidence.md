...
- **`AMBER_QUOTED_NO_FILL`** — the MM posted live commitments but no fill occurred in the window. `live-commitments-posted` is `proven_live`; `live-fill` is `deferred` (or `failed` with evidence); the fill-telemetry row `own-state-sse-canonical-fill` is `deferred`, `failed`, or `not_applicable` (a proven canonical fill would imply a fill occurred); and no successful `match-commitment` transaction appears (that would be a fill). Artifact status: `partial` or `complete_verified_with_caveats`.
