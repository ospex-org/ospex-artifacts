...
See `docs/artifact-types.md` for the current conventions and `docs/test-harness-expectations.md` for live-write harness expectations. New market-maker live canary runs publish a standardized scenario matrix and MVE readiness scorecard (pre-scheme runs are grandfathered); see `docs/mm-live-canary-evidence.md` and the templates under `templates/mm-live-canary/`.
